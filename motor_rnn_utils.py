...
class RNN:
    ...
    def relearn(self, trials, ext, ntstart, decoder, feedback, target, 
        ...
        self.P = [1.0/delta * np.eye(len(self.W_plastic[i])) 
...
def create_reaching_task_stimuli(tsteps: int, pulse_steps: int, 
                                n_targets: int = 6, amplitude: float = 1.0,
                                twod: bool = False) -> np.ndarray:
    """
...
def create_reaching_task_targets(tsteps: int, pulse_steps: int,
                               n_targets: int = 6, stype: str = 'constant',
                               target_max: float = 0.2) -> np.ndarray:
    """
...
def create_reaching_task_decoder(network: RNN, n_output_units: int = 2,
                               target_max: float = 0.2) -> np.ndarray:
    """
...
def train_reaching_decoder(neural_activity: np.ndarray, targets: np.ndarray,
                         trial_order: np.ndarray, 
                         n_output_units: int = 2) -> Tuple[np.ndarray, float]:
    """
...
def get_manifold(network: RNN, trials: int, stimulus: np.ndarray,
                pulse_length: int) -> Dict:
    """
...
def transform_reaching(reaching_network: RNN, manifold_out: Dict,
                      W_bci: np.ndarray, n_output_units: int,
                      reduced_dim: int) -> np.ndarray:
    """
...
def get_cost(result: np.ndarray, target: np.ndarray, 
...
def simulate_reaching(savedir: str, dt: float) -> np.ndarray:
    ...
    for trial in range(n_trials):
        plt.plot(positions[trial, :, 0], positions[trial, :, 1], 
                color=colors[trial], alpha=0.7, linewidth=2)
...
def plot_reaching_task_stimuli(stimulus: np.ndarray, n_targets: int,
                             tsteps: int, T: float):
    """Visualize stimulus structure as heatmaps."""
    fig, axes = plt.subplots(n_targets, 1, figsize=(12, 6))
    
    for target in range(n_targets):
        im = axes[target].imshow(stimulus[target, :, :].T, 
    ...
    fig.suptitle("Stimulus Structure: One-hot encoded directional cues", 
                 fontsize=12)
    plt.tight_layout()
...
def plot_reaching_task_targets(target: np.ndarray, tsteps: int, T: float):
    ...
    for target_idx in range(n_targets):
        ...
        axes[0].plot(target[target_idx, :, 0], color=color, 
...
def create_force_task_stimuli(tsteps: int, pulse_steps: int,
                            n_targets: int = 1, amplitude: float = 1.0,
                            twod: bool = False) -> np.ndarray:
...
def create_force_task_targets(tsteps: int, pulse_steps: int,
